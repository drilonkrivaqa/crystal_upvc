...
double _fallbackHeightMm(double? providedHeight) {
...
double _fallbackWidthMm(double? providedWidth, double heightMm) {
...
Future<Uint8List?> generateWindowDoorDesignPreview({
  required int rows,
  required int cols,
  ...
  double maxPreviewPx = 600,
  double pixelRatio = 3,
}) async {
  final safeRows = rows.clamp(1, 8);
  final safeCols = cols.clamp(1, 8);
  final windowHeightMm = _fallbackHeightMm(heightMm);
  final windowWidthMm = _fallbackWidthMm(widthMm, windowHeightMm);
  final aspectRatio =
      windowHeightMm > 0 ? windowWidthMm / windowHeightMm : 1.0;
  final heightPx = aspectRatio >= 1 ? maxPreviewPx / aspectRatio : maxPreviewPx;
  final widthPx = aspectRatio >= 1 ? maxPreviewPx : maxPreviewPx * aspectRatio;
  final targetSize = Size(widthPx, heightPx);

  final total = safeRows * safeCols;
  final effectiveCells = cells.length == total
      ? List<SashType>.from(cells, growable: false)
      : List<SashType>.filled(total, SashType.fixed, growable: false);
  final glassColor = glassColorForIndex(glassColorIndex).color;
  final cellGlassColors =
      List<Color>.filled(total, glassColor, growable: false);
  final profileColor = profileColorForIndex(profileColorIndex);
  final blindColor = blindColorForIndex(null);
  final columnFractions =
      _normalizedFractionsForPreview(columnSizes, safeCols);
  final rowFractions = _normalizedFractionsForPreview(rowSizes, safeRows);
  ...
  painter.paint(canvas, targetSize);
  final picture = recorder.endRecording();
  final image = await picture.toImage(
    (targetSize.width * pixelRatio).round(),
    (targetSize.height * pixelRatio).round(),
  );
