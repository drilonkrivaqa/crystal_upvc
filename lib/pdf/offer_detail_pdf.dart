...
/// Builds a PDF that imitates the UI of [OfferDetailPage] and returns the raw
/// PDF bytes.
Future<Uint8List> buildOfferDetailPdf({
    ...
    if (path != null && path.isNotEmpty) {
      try {
        ...
        if (path.startsWith('http')) {
          bytes = await networkImage(path) as Uint8List;
        } else {
    ...
    pw.MultiPage(
      ...
      build: (context) {
        ...
        for (var i = 0; i < offer.items.length; i++) {
          ...
          final finalPrice =
              item.manualPrice ?? total * (1 + offer.profitPercent / 100);
          ...
          final details = <pw.Widget>[
            ...
            if (blind != null)
              pw.Text('Blind: ${blind.name}, ${currency.format(blindCost)}'),
            if (mechanism != null)
              pw.Text('Mechanism: ${mechanism.name}, ${currency.format(mechanismCost)}'),
            if (accessory != null)
              pw.Text('Accessory: ${accessory.name}, ${currency.format(accessoryCost)}'),
            ...
            pw.Text('Profile cost: ${currency.format(profileCost)}'),
            pw.Text('Glass cost: ${currency.format(glassCost)}'),
            if (blind != null)
              pw.Text('Blind cost: ${currency.format(blindCost)}'),
            if (mechanism != null)
              pw.Text('Mechanism cost: ${currency.format(mechanismCost)}'),
            if (accessory != null)
              pw.Text('Accessory cost: ${currency.format(accessoryCost)}'),
            if (item.extra1Price != null)
              pw.Text('${item.extra1Desc ?? 'Additional 1'}: ${currency.format(item.extra1Price)}'),
            if (item.extra2Price != null)
              pw.Text('${item.extra2Desc ?? 'Additional 2'}: ${currency.format(item.extra2Price)}'),
            pw.Text('Cost (0%): ${currency.format(total)}'),
            pw.Text('With profit: ${currency.format(finalPrice)}'),
            pw.Text('Profit: ${currency.format(profitAmount)}'),
          ];
...
/// Generates the PDF for an offer detail and immediately shares it using the
/// platform share dialog.
Future<void> shareOfferDetailPdf({
  required Offer offer,
  required int offerNumber,
  required Box<Customer> customerBox,
  required Box<ProfileSet> profileSetBox,
  ...
  final bytes = await buildOfferDetailPdf(
    offer: offer,
